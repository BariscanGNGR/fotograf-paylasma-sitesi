	  ...
	  if(isset($_SESSION['kullaniciAdi']) && isset($_SESSION['sifre']))
	  {
		  ...
		  if($satirSayi > 0 && $satir['sifre']==$sifre && strtolower( $satir['kullanici-adi'] )== strtolower($kullaniciAdi))
		  {
			  ...
			  if(isset($_GET['sirala']))
			  {
				  ...
				  {
					  sorgula("SELECT * FROM `fotograf` WHERE `kullanici-id`=".get_kullaniciid($_SESSION['kullaniciAdi'])." ORDER BY `foto-id` DESC");
				  }
				  else if($_GET['siralama']=='Tarih Eskiden Yeniye')
				  {
					  sorgula("SELECT * FROM `fotograf` WHERE `kullanici-id`=".get_kullaniciid($_SESSION['kullaniciAdi'])." ORDER BY `foto-id` ASC");
				  }
